...
const frequencyStyles: Record<Medicine["frequency"], string> = {
  ...
  as_needed: "bg-orange-50 text-orange-700",
};
...
export function MedicineCard({ medicine, onEdit, onDelete }: MedicineCardProps) {
  return (
    <div className="flex h-full flex-col divide-y divide-gray-100 overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm transition-shadow hover:shadow-md">
      <div className="flex items-start justify-between gap-3 px-5 py-4 sm:px-6">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-sm text-gray-400">{medicine.dosage}</p>
        </div>
        <span
      ...
      {medicine.notes && (
        <div className="px-5 py-4 text-sm leading-relaxed text-gray-600 sm:px-6">
      ...
      <div className="mt-auto flex gap-2 px-5 py-4 sm:px-6">
        ...
        <button
          ...
          className="flex-1 rounded-lg bg-red-50 px-3 py-2 text-sm font-semibold text-red-600 transition-colors hover:bg-red-100"
        >
          Delete
        </button>
      </div>
