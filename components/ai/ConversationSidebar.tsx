...
export function ConversationSidebar({ groups, onNewConversation }: ConversationSidebarProps) {
  return (
    <div className="flex w-full flex-col gap-4 border-b border-gray-100 pb-4 dark:border-gray-800 lg:w-64 lg:shrink-0 lg:border-b-0 lg:border-r lg:pb-0 lg:pr-4">
      <button
      ...
      <div className="flex flex-col gap-3 overflow-y-auto lg:max-h-[28rem]">
        {groups.length === 0 ? (
          <p className="text-xs text-gray-400 dark:text-gray-500">No conversation history yet.</p>
        ) : (
          groups.map((group) => {
            ...
            return (
              <div key={group.label}>
                ...
                {lastMessage && (
                  <p className="truncate text-sm text-gray-600 dark:text-gray-400">{lastMessage.content}</p>
                )}
              </div>
            );
          })
