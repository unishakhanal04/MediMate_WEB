...
export function PrescriptionChart({ report }: PrescriptionChartProps) {
  ...
  return (
    <Card className="flex flex-col gap-4">
      <h2 className="text-base font-bold text-gray-900 dark:text-white">Prescriptions</h2>

      <div className="flex items-center gap-3">
        <div className="h-2 flex-1 overflow-hidden rounded-full bg-red-100 dark:bg-red-500/20">
          <div className="h-full rounded-full bg-emerald-500" style={{ width: `${activeRatio}%` }} />
        </div>
        <span className="text-xs font-semibold text-gray-500 dark:text-gray-400">{activeRatio}% active</span>
      </div>
      ...
      {report.recentPrescriptions.length === 0 ? (
        <p className="py-4 text-sm text-gray-500 dark:text-gray-400">No prescriptions uploaded yet.</p>
      ) : (
          ...
          {report.recentPrescriptions.map((prescription) => (
            <div key={prescription.id} className="flex items-center justify-between gap-3 py-2">
              <div className="min-w-0">
                <p className="truncate text-sm font-medium text-gray-900 dark:text-white">{prescription.title}</p>
                <p className="text-xs text-gray-500 dark:text-gray-400">{prescription.doctorName}</p>
              </div>
